...
def LP(T,R,alpha,ter):
    ...
    for i in range(T.shape[0]):
        ...
        if i not in ter:
            ...
            V.append(p.LpVariable(string,lowBound=0,upBound=0)) #Geeks https://www.geeksforgeeks.org/python-linear-programming-in-pulp/
    Lp+= -1*sum(V)
    for i in range(T.shape[0]):
        if i not in ter:
            ...
            for j in range(T.shape[-1]):
                Lp+= V[i]>= cond[j]
    sol=Lp.solve(p.PULP_CBC_CMD(msg=0)) #https://stackoverflow.com/questions/64215540/how-to-do-model-solve-not-show-any-message-in-python-using-pulp
    soln=[]
    for i in V:
...
parser.add_argument('--algorithm',type=str,default='vi',required=False,help="mention a valid algorithm")
